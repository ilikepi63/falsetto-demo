...
<head>
    <title>Code coverage report for src/cql-generators/create-table.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/cql-generators</a> create-table.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-yes">6x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">6x</span></td><td class="text"><pre class="prettyprint lang-js">import { Attribute } from "../attributes";
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (query.clusteringColumns.length === 0) {
<span class="cstat-no" title="statement not covered" >        return `${PRIMARY_KEY} ${parenthesis(`${createPartitionKey(query.partitionKey.map(<span class="fstat-no" title="function not covered" >pkey </span>=&gt; <span class="cstat-no" title="statement not covered" >pkey.name)</span>)}`)}`;</span>
    }
&nbsp;
...
&nbsp;
export const createClusteringOrder = (clusteringKeys: Array&lt;ClusteringColumn&gt;): string =&gt; clusteringKeys.length &gt; 0 ? `${WITH_CLUSTERING_ORDER_BY} ${parenthesis(clusteringKeys.map(col =&gt; createClusteringColumn(col)).join(" "))}` : <span class="branch-1 cbranch-no" title="branch not covered" >"";</span></pre></td></tr></table></pre>
